...
def sleepnet_collate(batch: list) -> dict:
    ...
    for i, sample in enumerate(batch):
        ...
        time_vec = torch.tensor(range(0, sample["epoch_count"]), dtype=sample["ecgs"].dtype)
        time_vec *= 1.0 / ((3600 / 30) * 24)
        time_vec += sample["midnight_offset"] % 1.0
        time_vec *= 2.0 * torch.pi
        time_vec = time_vec.unsqueeze(1)

        # create circular time vectors
        cos_t_vec = torch.cos(time_vec)
        sin_t_vec = torch.sin(time_vec)

        # concat the the demographics, with the circular time vectors
        sample_additional = torch.cat((sample_additional, cos_t_vec, sin_t_vec), dim=1)
...
class Chomp1d(Module):
    ...
    def __init__(self, chomp_size: int):
        ...
        self.chomp_size = chomp_size  # only cut from end
...
class TemporalConvNet(Module):
    ...
    def __init__(self, layer_count: int, channel_list, dropout: float = 0.0):
        ...
        assert len(channel_list) == 3
        kernel_size = 2  # fixed for TCN (always 3) 2 for causal-only
